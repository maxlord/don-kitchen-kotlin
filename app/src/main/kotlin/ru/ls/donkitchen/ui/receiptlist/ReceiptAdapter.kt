 ...
 * @since 17.10.16
 */
class ReceiptAdapter(private val context: Context, private val callback: Callback) :
	androidx.recyclerview.widget.RecyclerView.Adapter<ReceiptAdapter.ViewHolder>() {
	interface Callback {
		fun onItemClick(item: ReceiptViewItem)
	}

	class ViewHolder(view: View, adapter: ReceiptAdapter) :
	...
	override fun onBindViewHolder(holder: ViewHolder, position: Int) {
		...
		Glide.with(context).load(item.imageLink).fitCenter().centerCrop()
